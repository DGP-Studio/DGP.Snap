using DGP.Snap.UI.Extensions;
using System;
using System.Linq;
using System.Windows;
using System.Windows.Media.Animation;

namespace DGP.Snap.UI.Control
{
    public class ProgressBar : System.Windows.Controls.ProgressBar
    {
        ...
        private void VisibleChangedHandler(object sender, DependencyPropertyChangedEventArgs eventArgs)
        {
            // reset Storyboard if Visibility is set to Visible #1300
            if (this.IsIndeterminate)
            {
                ToggleIndeterminate(this, (bool)eventArgs.OldValue, (bool)eventArgs.NewValue);
            }
        }

        private static void OnIsIndeterminateChanged(DependencyObject dependencyObject, DependencyPropertyChangedEventArgs eventArgs)
        {
            var bar = (ProgressBar)dependencyObject;
            if (bar.IsLoaded && bar.IsVisible)
            {
                ToggleIndeterminate(bar, (bool)eventArgs.OldValue, (bool)eventArgs.NewValue);
            }
        }

        private static void ToggleIndeterminate(ProgressBar bar, bool oldValue, bool newValue)
        {
        ...
        private void SizeChangedHandler(object sender, SizeChangedEventArgs eventArgs)
        {
        ...
        private double ActualSize(bool invalidateMeasureArrange)
        {
            ...
            return this.Orientation == System.Windows.Controls.Orientation.Horizontal ? this.ActualWidth : this.ActualHeight;
        }
            ...
            lock (this.lockme)
            {
                ...
                try
                {
                    ...
                    if (indeterminate != null && this.indeterminateStoryboard != null)
                    {
                        ...
                        var doubleAnim = newStoryboard.Children.First(timeLine => timeLine.Name == "MainDoubleAnim");
                        ...
                        {
                            var doubleAnimParent = (DoubleAnimationUsingKeyFrames)newStoryboard.Children.First(timeLine => timeLine.Name == elemName + "Anim");
